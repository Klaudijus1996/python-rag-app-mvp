...
class TestRAGSystem:
    ...
    def test_rag_system_initialization(
        self, mock_vector_store, mock_openai, monkeypatch
    ):
...
class TestLoadRetriever:
    ...
    def test_load_retriever_with_factory(self, mock_vector_store, monkeypatch):
        ...
        mock_vector_store.get_retriever.assert_called_once_with(
            search_type="mmr", k=10, lambda_mult=0.4
        )
        assert retriever is not None

    def test_load_retriever_with_similarity_search(
        ...
        mock_vector_store.get_retriever.assert_called_once_with(
            search_type="similarity", k=5, lambda_mult=None
        )
        assert retriever is not None
