...
class TestIngestFunctions:
    ...
    def test_load_and_process_data_success(
        self, mock_read_csv, mock_exists, sample_dataframe
    ):
...
class TestCreateAndSaveVectorStore:
    ...
    def test_create_and_save_vector_store_faiss(
        self, mock_embeddings_class, mock_factory, sample_documents
    ):
        ...
        mock_vector_store.create_from_documents.assert_called_once_with(
    ...
    @patch("ingest.VectorStoreFactory.create_from_env")
    @patch("ingest.OpenAIEmbeddings")
    def test_create_and_save_vector_store_pinecone(
        self, mock_embeddings_class, mock_factory, sample_documents
    ):
    ...
    @patch("ingest.VectorStoreFactory.create_from_env")
    @patch("ingest.OpenAIEmbeddings")
    def test_create_and_save_vector_store_failure(
        self, mock_embeddings_class, mock_factory, sample_documents
    ):
    ...
    @patch("ingest.VectorStoreFactory.create_from_env")
    @patch("ingest.OpenAIEmbeddings")
    def test_create_and_save_vector_store_embeddings_failure(
        self, mock_embeddings_class, mock_factory, sample_documents
    ):
...
class TestMainIngestionPipeline:
    ...
    @patch("ingest.chunk_documents")
    @patch("ingest.create_and_save_vector_store")
    def test_main_vector_store_failure(
        self,
        mock_create_and_save,
        mock_chunk,
        mock_load,
...
class TestEnvironmentConfiguration:
    ...
    def test_default_configuration_values(self):
        ...
        assert ingest.DATA_PATH == os.getenv(
            "APP_TEST_DATA_PATH", "data/big-basket-products-20.csv"
        )
        assert ingest.EMBED_MODEL == os.getenv(
...
class TestErrorHandling:
    ...
    def test_corrupted_data_handling(self):
        ...
        with (
            patch("ingest.Path.exists", return_value=True),
    ...
    def test_empty_documents_list(self):
        ...
        with (
            patch("ingest.VectorStoreFactory.create_from_env") as mock_factory,
            patch("ingest.OpenAIEmbeddings"),
        ):
            mock_vector_store = Mock()
            mock_factory.return_value = mock_vector_store
...
class TestIntegrationWithVectorStores:
    ...
    def test_integration_with_pinecone(self, sample_documents, monkeypatch):
        ...
        with (
            ...
            # Test the integration
            with patch(
                "vector_stores.pinecone_store.PineconeVectorStoreWrapper._wait_for_index_ready"
            ):
                result = ingest.create_and_save_vector_store(sample_documents)
    ...
    def test_switching_between_vector_stores(
        self, temp_dir, sample_documents, monkeypatch
    ):
