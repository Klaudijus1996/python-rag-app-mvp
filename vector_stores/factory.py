    ...
    def create_vector_store(config: VectorStoreConfig) -> VectorStoreInterface:
        ...
        if store_type == "faiss":
            if not config.faiss_config:
                raise ValueError(
                    "FAISS configuration is required for FAISS vector store"
                )

            return FAISSVectorStore(
                index_dir=config.faiss_config.index_dir, embeddings=embeddings
            )

        elif store_type == "pinecone":
            if not config.pinecone_config:
                raise ValueError(
                    "Pinecone configuration is required for Pinecone vector store"
                )
