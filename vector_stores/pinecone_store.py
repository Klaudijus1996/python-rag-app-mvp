...
try:
    ...
    logger.warning(
        "Pinecone dependencies not available. Install langchain-pinecone to use Pinecone vector store."
    )
...
class PineconeVectorStoreWrapper(VectorStoreInterface):
    ...
    def __init__(self, index_name: str, embeddings: Any, config: dict):
        ...
        if not PINECONE_AVAILABLE:
            raise ImportError(
                "Pinecone dependencies not available. Install langchain-pinecone to use Pinecone."
            )
        ...
        logger.info(
            f"Initialized Pinecone vector store: index_name={index_name}, "
    ...
    def save(self) -> None:
        ...
        logger.info(
            "Pinecone vector store automatically persists data - no save operation needed"
        )
    ...
    def get_retriever(
        ...
        logger.info(
            f"Creating Pinecone retriever with search_type={search_type}, k={k}"
        )
        ...
        try:
            if search_type == "mmr":
                ...
                retriever = self.vector_store.as_retriever(
                    search_type="similarity", search_kwargs={"k": k, **kwargs}
                )
    ...
    def _wait_for_index_ready(self, timeout: int = 60) -> None:
        ...
        raise TimeoutError(
            f"Pinecone index '{self.index_name}' did not become ready within {timeout} seconds"
        )
