...
class PineconeConfig:
    ...
    def validate(self) -> None:
        ...
        if not self.api_key:
            raise ValueError(
                "PINECONE_API_KEY is required when using Pinecone vector store"
            )
        if not self.region:
            raise ValueError(
                "PINECONE_REGION is required when using Pinecone vector store"
            )
        if not self.cloud:
            raise ValueError(
                "PINECONE_CLOUD is required when using Pinecone vector store"
            )
        if not self.index_name:
            raise ValueError(
                "PINECONE_INDEX_NAME is required when using Pinecone vector store"
            )
...
class VectorStoreConfig:
    ...
    def from_env(cls) -> "VectorStoreConfig":
        ...
        if store_type == "faiss":
            ...
            logger.info(
                f"Pinecone config: index_name={config.pinecone_config.index_name}, "
            ...
            raise ValueError(
                f"Unsupported vector store type: {store_type}. Supported types: faiss, pinecone"
            )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.faiss_config:
            result.update({"index_dir": self.faiss_config.index_dir})
