...
class ChatRequest(BaseModel):
    ...
    query_type: Optional[QueryType] = Field(
        None, description="Optional hint about the type of query"
    )
    max_products: Optional[int] = Field(
        5, description="Maximum number of products to return", ge=1, le=20
    )
    price_range: Optional[Dict[str, float]] = Field(
        None, description="Optional price filtering with 'min' and 'max' keys"
    )
...
class ChatResponse(BaseModel):
    ...
    answer: str = Field(..., description="Generated response to the user's query")
    products: Optional[List[ProductInfo]] = Field(
        None, description="List of relevant products mentioned in the response"
    )
    query_type: Optional[QueryType] = Field(
        None, description="Detected or provided query type"
    )
    sources_count: Optional[int] = Field(
        None, description="Number of source documents used for the response"
    )
...
class IngestRequest(BaseModel):
    ...
    force_reindex: Optional[bool] = Field(
        False, description="Whether to force reindexing even if index exists"
    )
    chunk_size: Optional[int] = Field(
        1000, description="Text chunk size for document splitting", ge=100, le=2000
    )
    chunk_overlap: Optional[int] = Field(
        200, description="Overlap between text chunks", ge=0, le=500
    )
...
class IngestResponse(BaseModel):
    ...
    products_processed: Optional[int] = Field(
        None, description="Number of products processed from source data"
    )
    index_path: str = Field(..., description="Path where the vector index is stored")
    embedding_model: str = Field(..., description="Name of the embedding model used")
    processing_time_seconds: Optional[float] = Field(
        None, description="Time taken for the ingestion process"
    )
